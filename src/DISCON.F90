...
REAL(4)           :: CornerFreq    =       1.25903 !MC                   ! Corner frequency (-3dB point) in the recursive, single-pole, low-pass filter, rad/s. -- chosen to be 1/4 the blade edgewise natural frequency ( 1/4 of approx. 1Hz = 0.25Hz = 1.570796rad/s)
...
REAL(4)           :: PC_DT         = 0.0001!MC  !JASON:THIS CHANGED FOR ITI BARGE:      0.0001                    ! Communication interval for pitch  controller, sec.
...
REAL(4)           :: PC_MaxRat     =       0.122173048!MC                 ! Maximum pitch  rate (in absolute value) in pitch  controller, rad/s.
REAL(4)           :: PC_MinPit     =       0.020469756!MC                       ! Minimum pitch setting in pitch controller, rad.
REAL(4)           :: PC_RefSpd     =     119.3846154!MC                    ! Desired (reference) HSS speed for pitch controller, rad/s.
...
REAL(4)           :: VS_CtInSp     =      38.27180777!MC                   ! Transitional generator speed (HSS side) between regions 1 and 1 1/2, rad/s.
...
REAL(4)           :: VS_MaxTq      =   48195.87629!MC                      ! Maximum generator torque in Region 3 (HSS side), N-m. -- chosen to be 10% above VS_RtTq = 43.09355kNm
REAL(4)           :: VS_Rgn2K      =       2.081!MC                  ! Generator torque constant in Region 2 (HSS side), N-m/(rad/s)^2.
REAL(4)           :: VS_Rgn2Sp     =      91.21091                   ! Transitional generator speed (HSS side) between regions 1 1/2 and 2, rad/s.
REAL(4)           :: VS_Rgn3MP     =       0.037923048!MC                ! Minimum pitch angle at which the torque is computed as if we are in region 3 regardless of the generator speed, rad. -- chosen to be 1.0 degree above PC_MinPit
REAL(4)           :: VS_RtGnSp     =     118.1907692!MC                    ! Rated generator speed (HSS side), rad/s. -- chosen to be 99% of PC_RefSpd
REAL(4)           :: VS_RtPwr      = 3438775.51!MC                      ! Rated generator generator power in Region 3, Watts. -- chosen to be 5MW divided by the electrical generator efficiency of 94.4%
...
!!====================== Reading Controller parameters from a file for easy tuning!! =====================
!! MC:27/Mar/2018
open(unit=45, file="ControllerVars.txt")
...
IF ( iStatus == 0 )  THEN  ! .TRUE. if we're on the first call to the DLL
   ...
   aviFAIL  = 1
   ErrMsg   = 'Running with torque and pitch control modified from the '// &
              'NREL5MW baseline wind turbine(by J.Jonkman) '// &
              'Controller modified at UT Dallas by MC on 27/Mar/2017 '   // &
              'Control variables read in from ControllerVars.txt'
